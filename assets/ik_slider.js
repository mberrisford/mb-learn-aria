; (function ($, window, document, undefined) {
	...
	var pluginName = 'ik_slider',
		defaults = {
			...
			'nowValue': 0,
			'step': 1,
			'instructions': 'Use the right and left arrow keys to increase or decrease the value'
		};
		...
		if (!plugin.textfield.is(':text')) {
			...
			plugin.knob = $('<div/>')
				.attr({
					...
					'role': 'slider', // assign role
					'aria-valuemin': plugin.options.minValue, //set the slider min value
					'aria-valuemax': plugin.options.maxValue, // set slider max val
					'aria-valuenow': plugin.options.minValue, // set current value
					'aria-describedby': id + '_instructions' // add description
				})
				.addClass('ik_knob')
				.on('keydown', { 'plugin': plugin }, plugin.onKeyDown) //add keypress triggers
